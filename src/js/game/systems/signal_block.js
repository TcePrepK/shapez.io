...
export class SignalBlockSystem extends GameSystemWithFilter {
    constructor() {
        super([SignalBlockComponent]);

        this.root.signals.entityManuallyPlaced.add(entity =>
            this.editSignalBlock(entity, { deleteOnCancel: true })
        );
    }
    ...
    update() {
        ...
        for (let i = 0; i < this.allEntities.length; ++i) {
            ...
            const signalComp = entity.components.SignalBlock;
            pinsComp.slots[0].value = signalComp.signal;
            pinsComp.slots[1].value = signalComp.signal;
            pinsComp.slots[2].value = signalComp.signal;
            pinsComp.slots[3].value = signalComp.signal;
        }
    }
     ...
     */
    editSignalBlock(entity, { deleteOnCancel = true }) {
        if (!entity.components.SignalBlock) {
            return;
        }
        ...
        const closeHandler = () => {
            ...
            const signalComp = entityRef.components.SignalBlock;
            if (!signalComp) {
            ...
            if (itemInput.chosenItem) {
                ...
                signalComp.signal = itemInput.chosenItem;
            } else {
                signalComp.signal = this.parseSignalCode(signalValueInput.getValue());
            }
        };
